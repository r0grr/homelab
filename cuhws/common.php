...
if(isSet($_GET['lang']))
{
	$selected_lang_code = $_GET['lang'];
	
	// GET the session  set the cookie
	$_SESSION['lang'] = $selected_lang_code;
	
	setcookie("lang", $selected_lang_code, time() +3600);
}
else if(isSet($_SESSION['lang']))
{
	$selected_lang_code = $_SESSION['lang'];
}
else if(isSet($_COOKIE['lang']))
{
	$selected_lang_code = $_COOKIE['lang'];
}
else
{
...
if(!isset($LanguageList[$selected_lang_code])) { // Assume English if language not in current list
	$selected_lang_code = 'cat';
}
// Set the template options
list ($lang_file,$lang_flag,$lang_option,$language,$lang_locale) = explode('|',$LanguageList[$selected_lang_code]);
