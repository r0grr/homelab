...
//  $weather["temp_feel"]          = $cumulus[54]; // use Apparent Temperature
  ...
  $weather["temp_feel"]          = apparent_temperature($weather['temp'],$weather['humidity'],$weather['wind_speed']);
	...
	$weather["thsw"] = (isset($cumulus[58]) && $cumulus[58] !== '') ? floatval($cumulus[58]) : (isset($weather["heat_index"]) ? floatval($weather["heat_index"]) : floatval($weather["temp"]));
...
$o='Template by weather34.com';date_default_timezone_set($TZ);
if (!function_exists('isdayofmonth')) {
...
if(file_exists('jsondata/kindex.txt')) {
  ...
  if(is_array($json) && count($json) > 0) {
    $latest = end($json);
    if(is_array($latest) && isset($latest['Kp'])) {
      $kp = (float)$latest['Kp'];
    } elseif(is_array($latest) && isset($latest[1])) {
      $kp = (float)$latest[1];
    } else {
      $kp = 0;
    }
